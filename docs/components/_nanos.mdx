...
export const CardLink = ({ title, description, href, image_src }) => (
	<a class="card-link" href={href.startsWith("http") ? href : `http://localhost:3000/docs/${href}`} >
...
export const ReferenceLink = ({ children, href }) => (
	<a class="reference-link" href={href.startsWith("http") ? href : `http://localhost:3000/docs/${href}`}>
		{children}
		<span>
			{href}
		</span>
